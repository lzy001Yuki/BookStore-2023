...
void Book::modify(const char *isbn, const char *name, const char *author, const char* all_key, double price, UserAll &user_all,
    ...
    if (n_flag) {
        ...
        // 删除的时候已经进行了查找
        // 如果书名存在 isbn->name
        Node pre_name_isbn(target.BookName, now_user.select_isbn); // BookName可能为空
        book_name.Delete(pre_name_isbn);// 没有就不删除
        strcpy(target.BookName, name);// book_isbn更改
        Node new_name_isbn(target.BookName, now_user.select_isbn);
        book_name.Insert(new_name_isbn);
    }
    if (a_flag) {
        Node pre_author_isbn(target.Author, now_user.select_isbn);
        book_author.Delete(pre_author_isbn);
        strcpy(target.Author, author);// book_isbn更改
        Node new_author_isbn(target.Author, now_user.select_isbn);
    ...
    if (k_flag) {
        ...
        if (target.hasKey) {
            ...
            /// special delete!!!
            /// 为keyword新开一个类
            std::vector<std::string> ans2 = SplitWords(target.keyword);
            for (auto &it : ans2) {
                char ch[66] = {'\0'};
                strcpy(ch, it.c_str());
    ...
    if (I_flag){
        if (strcmp(target.ISBN, isbn) == 0) throw InvalidExp();
        book_isbn.Delete(target);
        strcpy(target.ISBN, isbn);
        strcpy(target.index, isbn);
        /// insert
        bool flag = book_isbn.Insert(target);
...
void Book::Import(int quantity, double total, UserAll &user_all, Finance &fin) {
    ...
    /// 和finance 关联
    bool sta = false;
    fin_info node(total, sta, now_user.UserID);
    int total_count;
...
void Book::buy(const char *isbn, int quantity, UserAll &user_all, Finance &fin) {
    ...
    fin.fin_report.write_info(total_count, 1);
    bool sta = true;
    fin_info node(cost, sta, now_user.UserID);
