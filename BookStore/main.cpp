...
bool processLine(std::string str, UserAll &user_all, Book &book, Finance &finance);
...
int main() {
    ...
    while (true) {
        ...
        try {
            if (!processLine(line, user_all, book, fin)) break;//return 0;
...
bool processLine(std::string str, UserAll &user_all, Book &book, Finance &finance) {
    ...
    if (command == "su") {
        ...
        if (parse.size() == 3) {
            ...
            user_all.su(userid, password);
        } else if (parse.size() == 2) {
            user_all.su(userid, nullptr);
        }
